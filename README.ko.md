...
- **자동 분류** — 분류를 확정할 때마다 핑거프린트 규칙(가맹점 + 금액 ±8%)을 학습.
  수동 등록 시 일치하는 규칙이 폼 기본값보다 우선해 자동 분류된다.
...
- `frontend/src/pages` — 대시보드 / 거래내역(수동 등록 포함) / 카드 / 정기결제 / 캐시백 / 통계 / 설정
...
- **카테고리(category)**: 용도. kind 가 수입/지출/이체를 구분 (급여·통신비·대출상환·회비·투자이체 등).
  **주/부 2단 계층**으로 관리한다(예: `식비 > 배달`). 거래는 주·부 어디에나 분류할 수 있고,
  통계·예산은 기본적으로 부 지출을 주로 합산해 보여준다(통계 탭의 주/부 토글로 부 단위 상세 전환).
  주에 예산을 걸면 하위 부 지출까지 합산해 초과를 판정한다.
  기존 `식비/배달` 같은 이름은 첫 접속 시 `식비 > 배달`로 자동 분리되는데,
  슬래시 앞부분이 이미 카테고리로 있을 때만 분리하므로 `회비/경조사` 같은 이름은 그대로 유지된다.
- **캐시백(cashback)**: 카드마다 결제액의 기본 적립률(예: 1%)과, 결제 후 정해진 기한 안에
  카드대금을 갚으면 얹어 주는 추가 적립률(예: 5일 내 +1%)을 둘 수 있다. 요율·기한은 카드 탭에서
  카드별로 설정하고, **캐시백 탭**에서 아직 받을 수 있는 건(남은 일수 표시)·획득분·놓친 분을 본다.
  그 화면에서 클릭 한 번으로 납부 처리한다.
- **결제수단(payment method)**: 어느 카드/현금/계좌로 나갔는가. 카드는 카드 탭에서 결제일/실적기간/실적한도 관리.
  거래에 **실적 제외** 표시를 걸 수 있다 — 세금·공과금처럼 카드사가 실적으로 인정하지 않는 결제용이다.
  수동 등록·수정 모달의 체크박스로 켜거나, 거래내역에서 여러 건을 골라 일괄 바에서 한 번에 처리한다
  제외 표시된 결제는 **카드 실적·실적 페이스에서만** 빠지고
  일반 지출 통계에는 그대로 잡히며, 카드 위젯에 제외 합계를 함께 보여줘 숫자가 맞아떨어지게 한다.
- **카드 혜택(card benefit)**: 카드사 안내에서 옮겨 적어 두는 **자료**다. 카드 탭에서 카드를 고르면
  연회비(국내/해외)·전월 실적 조건과 함께 `영역 / 구분(적립·할인·서비스) / 요율 / 월 한도 / 비고`
  표가 나오고, 여기서 추가·수정·삭제·순서 변경을 한다. 카드사 표기가 "최대 4.5%"처럼 실적 구간에 따라
  달라지는 경우가 많아 **최대치** 표시와, 상세 조건을 아직 확인하지 못한 줄에 붙이는 **확인 필요** 표시를
  함께 둔다 — 나중에 이 값을 얼마나 믿어도 되는지 알 수 있게 하기 위해서다.
  자료 출처 URL과 유의사항은 카드 등록 폼에서 입력한다. 앱이 이 값으로 적립액을 계산하지는 않는다
  (계산은 캐시백 설정이 담당한다).
